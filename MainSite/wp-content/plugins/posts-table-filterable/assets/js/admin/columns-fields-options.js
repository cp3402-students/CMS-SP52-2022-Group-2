...
document.addEventListener('tableon-call-popup', function (e) {
    let call_id = 'tableon_options_columns_table';
    if (e.detail.call_id === call_id) {
        new TABLEON_ColumnsFieldsOptions(JSON.parse(document.querySelector(`[data-table-id="${call_id}"]`).innerText), call_id);
    }
});

class TABLEON_ColumnsFieldsOptions extends TABLEON_GeneratedTables {
    constructor(table_data, table_html_id) {
        super(table_data, table_html_id);
        this.save_table_field_action = 'tableon_save_table_column_field_option';//reinit ajax action
        this.switcher_action = 'tableon_save_table_column_field_option';
    ...
    do_after_draw() {
        ...
        this.table.querySelectorAll('.tableon-color-field').forEach(item => {
    ...
    init_html_items_action(html_item_type, action) {
        this.table.querySelectorAll(`.table23_td_value ${html_item_type}.tableon-columns-field-option`).forEach(item => {
