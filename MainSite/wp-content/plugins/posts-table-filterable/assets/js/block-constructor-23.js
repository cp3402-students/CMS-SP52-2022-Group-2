...
class BlockConstructor23 {
    ...
    create_element(key, data) {
        ...
        if (data.has_settings) {
            ...
            btn.className = 'blocks-constructor-23-element-btn tableon-btn';
            ...
            btn.addEventListener('click', (e) => {
                ...
                let call_id = tableon_helper.create_id('tableon-');
                ...
                tableon_helper.call_popup('tableon_get_filter_field_options', args, call_id, tableon_helper_vars.lang.filter_field_popup_title.sprintf(this.additional.table_id, key), {left: 20, right: 20}, `<a href="https://posts-table.com/tableon-documentation/" class="tableon-btn" target="_blank">${tableon_helper_vars.lang.help}</a>`);

                document.addEventListener('tableon-call-popup', (e) => {
                    if (e.detail.call_id === call_id) {
                        new TABLEON_FilterFieldsOptions(JSON.parse(document.querySelector(`[data-table-id="${call_id}"]`).innerText), call_id);
                    }
                });
