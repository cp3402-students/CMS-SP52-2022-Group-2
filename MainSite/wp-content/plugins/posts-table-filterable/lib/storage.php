...
if (!class_exists('TABLEON_STORAGE')) {

    final class TABLEON_STORAGE {
        ...
        public function set_val($key, $value) {
            switch ($this->type) {
                case 'session':
                    $_SESSION[$key] = sanitize_text_field(esc_html__($value));
                    break;
                case 'transient':
                    ...
                    $data[$key] = sanitize_text_field(esc_html__($value));
                    set_transient($this->transient_key, $data, 1 * 24 * 3600); //1 day
                    break;
                case 'cookie':
                    setcookie($key, sanitize_text_field(esc_html__($value)), time() + 1 * 24 * 3600); //1 day
                    break;
