...
class TABLEON_Vocabulary {

    public static $translations = [];
    public $action = 'tableon_vocabulary_table';
    private $db_table = 'tableon_vocabulary';
    private $db = null;
    ...
    public function __construct() {
        ...
        //WPML compatibility
        add_filter('tableon_current_lang', function($lang) {
    ...
    public function admin_enqueue_scripts() {
        if (isset($_GET['page']) AND $_GET['page'] == 'tableon') {
            wp_enqueue_script('tableon-vocabulary', TABLEON_ASSETS_LINK . 'js/admin/vocabulary.js', ['tableon-generated-tables'], TABLEON_VERSION, true);
        }
    }
    ...
    public function admin_init() {
        if (TABLEON_HELPER::can_manage_data()) {
            add_action('wp_ajax_tableon_save_vocabulary_field', array($this, 'save'));
            add_action('wp_ajax_tableon_create_vocabulary_field', array($this, 'create'));
            add_action('wp_ajax_tableon_delete_vocabulary_field', array($this, 'delete'));
            ...
            add_action($this->action, function () {
                $profile = [
                    ...
                    'title' => [
                        'title' => esc_html__('Key Word', 'posts-table-filterable'),
                        'editable' => 'textinput',
                        'order' => 'asc'
                    ]
                ];
                ...
                if (self::is_enabled()) {
                    $languages = explode(',', TABLEON_Settings::get('languages'));
                    if (!empty($languages)) {
    ...
    public static function is_enabled() {
        return intval(TABLEON_Settings::get('languages')) > -1 AND!empty(TABLEON_Settings::get('languages'));
    }
    ...
    public function draw_table() {
        $table_html_id = 'tableon-vocabulary-table';
        ...
        echo TABLEON_HELPER::render_html('views/table.php', array(
            ...
            'placeholder' => esc_html__('search by keyword', 'posts-table-filterable') . ' ...',
            ...
            'table_data' => array_values(self::$translations)
                ], $table_html_id, 'tableon-vocabulary-json-data');
    }
    ...
    private function get_data() {
        ...
        if (!empty($res)) {
            foreach ($res as $v) {
                ...
                $tmp['delete'] = TABLEON_HELPER::draw_html_item('a', [
                            'href' => "javascript: tableon_vocabulary_table.delete({$v['id']});void(0);",
    ...
    public function save() {
        ...
        if ($post_id) {
            ...
            if ($_REQUEST['field'] !== 'title') {
                ...
                $translations[TABLEON_HELPER::sanitize_text($_REQUEST['field'])] = TABLEON_HELPER::sanitize_text($_REQUEST['value']);
    ...
    public function create() {
        $this->db->insert($this->db_table, [
            'title' => '0 __' . __('new translate', 'posts-table-filterable') . ' ' . TABLEON_HELPER::sanitize_text($_REQUEST['tail'])
        ]);
    ...
    public static function get($title_key, $lang = '') {
        ...
        if (isset(self::$translations[$title_key]) AND!empty(self::$translations[$title_key])) {
            if (empty($lang)) {
                $lang = apply_filters('tableon_current_lang', get_locale());
            }
