 ...
 * TABLEON Meta
 *
 * Handles posts metadata on admin panel side
 *
 * @since   1.0.0
 */
if (!defined('ABSPATH')) {
...
class TABLEON_TablesMeta {

    private $db_table = 'tableon_tables_meta';
    private $db = null;
    ...
    public function __construct() {
        ...
        add_filter('tableon_table_orderby_select_args', function($args, $table_id = 0) {

            if ($table_id > 0) {
                $found_rows = $this->get_rows($table_id);

                if (!empty($found_rows)) {
                    $table_columns = tableon()->columns->get_table_columns(intval($table_id), ['fields' => 'title,field_key']);
                    foreach ($found_rows as $r) {
                        ...
                        $args[$r['meta_key']] = $title . ': ' . esc_html__('Ascending', 'posts-table-filterable');
                        $args[$r['meta_key'] . '-desc'] = $title . ': ' . esc_html__('Descending', 'posts-table-filterable');
                    }
                }
            }
    ...
    public function admin_enqueue_scripts() {
        if (isset($_GET['page']) AND $_GET['page'] == 'tableon') {
            wp_enqueue_script('tableon-tables-meta', TABLEON_ASSETS_LINK . 'js/admin/tables-meta.js', ['data-table-23'], TABLEON_VERSION, true);
        }
    }
    ...
    public function admin_init() {
        if (TABLEON_HELPER::can_manage_data()) {
            ...
            add_action('wp_ajax_tableon_get_tables_meta', array($this, 'draw_table'));
            add_action('wp_ajax_tableon_save_table_meta_field', array($this, 'save'));
            add_action('wp_ajax_tableon_create_meta', array($this, 'create'));
            add_action('wp_ajax_tableon_delete_table_meta', array($this, 'delete'));
        }
    }

    /**
     * Extend default profile by added meta fields
     *
     * @since 1.0.0
     ...
     * Useful for such shortcodes as [tableon_favourites]
     *
     * @since 1.0.2
     * @param array $m
     * @param int $table_id table ID
    ...
    public function assemble_meta_for_system($m, $table_id = 0) {
        $tmp = [
            ...
            'meta_key' => $m['meta_key'],
            'filter' => true,
            'action' => function($post_id) use($m, $table_id) {
                return apply_filters('tableon_meta_data_cell', get_post_meta($post_id, $m['meta_key'], true), $m, $post_id, $table_id);
            }
        ];

        switch ($m['meta_type']) {
            case 'text':
                $tmp['options'] = TABLEON_Default::$fields_options;
                $tmp['order'] = 'asc';
            ...
            case 'calendar':
                ...
                $tmp['action'] = function($post_id) use($m, $table_id) {
                    $val = apply_filters('tableon_meta_data_cell', get_post_meta($post_id, $m['meta_key'], true), $m, $post_id, $table_id);

                    if (is_string($val)) {
                        $val = strtotime($val);
                    }

                    return date(apply_filters('tableon_date_format', get_option('date_format')), $val);
                };
                ...
                $tmp['get_filter_query_args'] = function($args, $value, $is_calendar_dir_to = false) use($m, $table_id) {
                    ...
                    $calendar_type = tableon()->filter->fields_options->get($table_id, $fkey . '-data-type');
        ...
        $tmp['get_filter_draw_data'] = function($table_id) use($m) {
            return tableon()->filter->get_field_drawing_data($table_id, $m['meta_key']);
        };
    ...
    public function draw_table() {
        $table_html_id = 'tableon_meta_table';
        echo tableon()->draw_table_data([
            'mode' => 'json',
            'action' => 'tableon_meta_table',
            'orderby' => 'meta_key',
            'order' => 'asc',
    ...
    public function add_table_action() {
        add_action('tableon_meta_table', function () {
            return [
                0 => [
                //'ajax_action' => ''
                ],
                ...
                'meta_key' => [
                    'title' => esc_html__('Meta key', 'posts-table-filterable'),
                    'editable' => 'textinput',
                    'order' => 'asc'
                ],
                'meta_type' => [
                    'title' => esc_html__('Type', 'posts-table-filterable'),
                    'editable' => 'select',
                    'order' => 'asc'
                ],
                'notes' => [
    ...
    private function get_prepared_data($table_id) {
        $columns = [];
        $fields = array_keys(apply_filters('tableon_meta_table', NULL));
                ...
                foreach ($fields as $field) {
                    switch ($field) {
                        ...
                        case 'meta_type':
                            ...
                            $tmp[$field] = TABLEON_HELPER::draw_select([
                                        'style' => 'width: 100%;'
                                            ], [
                                        'not_defined' => esc_html__('not defined', 'posts-table-filterable'),
                                        'text' => esc_html__('text', 'posts-table-filterable'),
                                        'number' => esc_html__('number', 'posts-table-filterable'),
                                        'calendar' => esc_html__('calendar', 'posts-table-filterable')
                                            ], $r['meta_type']);
                        ...
                        case 'actions':
                            ...
                            $tmp[$field] = TABLEON_HELPER::draw_html_item('a', [
                                        'href' => "javascript: tableon_meta_table.delete({$r['id']});void(0);",
    ...
    /**
     * Save meta field option by ajax
     *
     * @since 1.0.0
     * 
     * @return void
    ...
    public function save() {
        $this->update_field(TABLEON_HELPER::sanitize_array($_REQUEST['field']), intval($_REQUEST['post_id']), TABLEON_HELPER::sanitize_text($_REQUEST['value']));
    ...
    public function create() {
        ...
        if ($table_id > 0) {
            $this->insert(array(
                'table_id' => $table_id,
                'meta_key' => esc_html__('0 write key here', 'posts-table-filterable')
            ));
        }
