...
class TABLEON_Columns {

    private $db_table = 'tableon_tables_columns';
    private $db = null;
    public $options = null;
    public $meta = null;
    public $filter = null;
    ...
    public function __construct() {
        ...
        $this->options = new TABLEON_TablesOptions();
        $this->filter = new TABLEON_TablesFilter();
        $this->meta = new TABLEON_TablesMeta();
    ...
    public function admin_enqueue_scripts() {
        if (isset($_GET['page']) AND $_GET['page'] == 'tableon') {
            wp_enqueue_script('tableon-columns', TABLEON_ASSETS_LINK . 'js/admin/columns.js', [], TABLEON_VERSION, true);
        }
    }
    ...
    public function admin_init() {
        if (TABLEON_HELPER::can_manage_data()) {
            ...
            add_action('wp_ajax_tableon_get_columns_data', array($this, 'get_columns_data'));
            add_action('wp_ajax_tableon_save_table_column_field', array($this, 'save_column_field'));
            add_action('wp_ajax_tableon_create_table_column', array($this, 'create_column'));
            add_action('wp_ajax_tableon_refresh_columns_table', array($this, 'refresh'));
            add_action('wp_ajax_tableon_delete_table_column', array($this, 'delete'));
        }
    }
    ...
    public function add_table_action() {
        add_action('tableon_columns_table', function () {
            return [
                0 => [
                //'ajax_action' => 'tableon_get_table_columns'
                ],
                'move' => [
                    'title' => esc_html__('Move', 'posts-table-filterable')
                ],
                'title' => [
                ...
                'is_active' => [
                    'title' => esc_html__('Active', 'posts-table-filterable'),
                    'order' => 'desc'
                ],
                'show_on_mobile' => [
                    'title' => esc_html__('Mobile', 'posts-table-filterable'),
                    'order' => 'desc'
                ],
                'field_key' => [
                    'title' => esc_html__('Field', 'posts-table-filterable'),
                    'editable' => 'select'
                ],
                'width' => [
                ...
                'notes' => [
                    'title' => esc_html__('Notes', 'posts-table-filterable'),
                    'editable' => 'textinput'
                ],
                'actions' => [
    ...
    private function get_table_columns_data($table_id, $order_by_active = true) {
        $columns = [];
        $fields = array_keys(apply_filters('tableon_columns_table', NULL));
        ...
        $table_action = TABLEON::get_table_action($table_id);
            ...
            foreach ($found_columns as $c) {
                ...
                foreach ($fields as $field) {
                    switch ($field) {
                        ...
                        case 'move':
                            $tmp[$field] = TABLEON_HELPER::draw_html_item('img', [
                                        'src' => TABLEON_ASSETS_LINK . 'img/move.png',
                                        'width' => 20,
                                        'alt' => esc_html__('drag and drope', 'posts-table-filterable'),
                                        'class' => 'tableon-tr-drag-and-drope'
                            ]);
                            break;
                        ...
                        case 'is_active':
                            $tmp[$field] = TABLEON_HELPER::draw_switcher('is_active', $c['is_active'], $c['id'], 'tableon_save_table_column_field');
                            break;

                        case 'show_on_mobile':
                            $tmp[$field] = TABLEON_HELPER::draw_switcher('show_on_mobile', $c['show_on_mobile'], $c['id'], 'tableon_save_table_column_field');
                            break;
                        ...
                        case 'field_key':
                            ...
                            if (!empty($profile)) {
                                $options = [
                                    0 => esc_html__('not selected', 'posts-table-filterable')
                                ];
                                ...
                                $tmp[$field] = TABLEON_HELPER::draw_select([], $options, $c[$field]);
                            } else {
                                //$tmp[$field] = esc_html__('select table action', 'posts-table-filterable');
                            }

                            if (!isset($tmp[$field])) {
                                $tmp[$field] = esc_html__('no fields found', 'posts-table-filterable');
                            }
                            break;

                        case 'actions':

                            $edit_popup_title = sprintf(esc_html__('Table: %s. Field: %s', 'posts-table-filterable'), addslashes(tableon()->tables->get($table_id)['title']), $c['title']);

                            //***                 

                            $help = esc_html__('Help', 'posts-table-filterable');
                            $edit_button = TABLEON_HELPER::draw_html_item('a', [
                                        'href' => "javascript: tableon_helper.call_popup(\"tableon_show_column_field_option\",{field_id:{$c['id']}, table_id:{$table_id}, not_paste:1},\"tableon_options_columns_table\",\"{$edit_popup_title}\", {left:15, right:15}, `<a href=\"https://posts-table.com/tableon-documentation\" class=\"tableon-btn\" target=\"_blank\">{$help}</a>`); void(0);",
                                        'title' => esc_html__('edit', 'posts-table-filterable'),
                                        'class' => 'button tableon-field-edit-btn tableon-dash-btn-single'
                                            ], '<span class="dashicons-before dashicons-edit"></span>');

                            //***

                            $tmp[$field] = $edit_button . TABLEON_HELPER::draw_html_item('a', [
                                        'href' => "javascript: tableon_columns_table.delete({$c['id']});void(0);",
                                        'title' => esc_html__('delete', 'posts-table-filterable'),
                        ...
                        default:
                            ...
                            $tmp[$field] = esc_html__('Wrong type', 'posts-table-filterable');
    ...
    public function get_columns_data() {
        $table_html_id = TABLEON_HELPER::sanitize_text($_REQUEST['table_html_id']);
        ...
        echo TABLEON_HELPER::render_html('views/table.php', array(
            ...
            'placeholder' => esc_html__('search by column title', 'posts-table-filterable') . ' ...',
            'classes' => 'tableon-columns-table',
            'table_view' => 'separated'
        )) . tableon()->draw_table_data([
            'mode' => 'json',
            'action' => 'tableon_columns_table',
    ...
    public function save_column_field() {
        ...
        switch ($_REQUEST['field']) {
            case 'pos_num':
                $ids = explode(',', TABLEON_HELPER::sanitize_array($_REQUEST['value']));
            ...
            default:
                ...
                $this->update_field(TABLEON_HELPER::sanitize_array($_REQUEST['field']), TABLEON_HELPER::sanitize_array($_REQUEST['post_id']), TABLEON_HELPER::sanitize_text($_REQUEST['value']));
                break;
        }
     ...
     * @return void
     */
    public function create($table_id, $prepend = false, $title = '', $field_key = NULL, $notes = '') {
        ...
        if (empty($title)) {
            $title = esc_html__('New Column', 'posts-table-filterable');
        }
                                ...
                                if (substr($key, 0, 4) === 'css-') {
                                    ...
                                    if ($css_key === 'media-hide') {
                                        ...
                                        if ($css_key === 'color') {
                                            ...
                                            $col_styles .= "#{$table_html_id} table .table23_td_{$css['field_key']} .tableon-num-cell{{$css_key}:{$value} {$important};} ";
                                        }
                                    }
                                }
                            }
