 ...
 * TABLEON Filter
 *
 * Handles posts filtration for shortcode [tableon] on the front
 *
 * @since   1.0.0
 */
if (!defined('ABSPATH')) {
...
include_once TABLEON_PATH . 'classes/filter-fields-options.php';
...
class TABLEON_Filter {
    ...
    public function __construct() {
        $this->fields_options = new TABLEON_FilterFieldsOptions();
    }
    ...
    public function get_taxonomy_drawing_data($taxonomy, $table_id = 0) {
        ...
        if (isset($wp_taxonomies[$taxonomy])) {
            ...
            if (!empty($terms)) {
                ...
                $filter_data = [
                    'title' => TABLEON_Vocabulary::get($wp_taxonomies[$taxonomy]->label),
                    'view' => $view,
                    'options' => []
                ];
     ...
     * @param string $taxonomy
     * @param array $terms
     * @param array $exclude exlude terms ids
        ...
        foreach ($terms as $t) {
            if (is_object($t)) {
                ...
                $options[] = [
                    ...
                    'name' => $t->name,
                    'slug' => $t->slug,
                    'title' => str_repeat('-', $level) . ' ' . TABLEON_Vocabulary::get($t->name)
                ];
    ...
    public function draw_filter_form_data($filter_args, $table_id) {
        ...
        if ($table_id > 0 AND empty($filter_args)) {
            $ak = tableon()->columns->filter->get_acceptor_keys($table_id);
                ...
                foreach ($filter_args as $k) {
                    ...
                    if (isset($filter_data[$k])) {
                        ...
                        if (isset($tmp[$k]['view']) AND $tmp[$k]['view'] === 'calendar') {
                            ...
                            $tmp[$k]['title'] = $tmp[$k]['placeholder'] = TABLEON_Vocabulary::get(sprintf(esc_html__('From: %s', 'posts-table-filterable'), $placeholder));
                            $tmp[$k . '---to']['title'] = $tmp[$k . '---to']['placeholder'] = TABLEON_Vocabulary::get(sprintf(esc_html__('To: %s', 'posts-table-filterable'), $placeholder));
                        }
                    }
                }
    ...
    public function get_field_drawing_data($table_id, $field_key) {
        ...
        if (empty($available_fields)) {
            $available_fields = apply_filters(TABLEON::get_table_action($table_id), $table_id);
        }
            ...
            if (!empty($field_options)) {
                foreach ($field_options as $option_key) {
                    ...
                    switch ($option_key) {
                        ...
                        default:
                            $res[$option_key] = TABLEON_Vocabulary::get($this->fields_options->get($table_id, "{$field_key}-{$option_key}"));
                            break;
                    }
                }
            }
        ...
        if (!isset($res['title']) OR!boolval(trim($res['title']))) {
            $profile = apply_filters(TABLEON::get_table_action($table_id), $table_id);
            $res['title'] = TABLEON_Vocabulary::get($profile[$field_key]['title']);
        }
